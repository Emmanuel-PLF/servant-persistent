...
userApp cfg = cors (const $ Just simpleCorsResourcePolicy) $ serve userApi (appToServer cfg)
...
app cfg =
  cors (const $ Just simpleCorsResourcePolicy) $ serve appApi (appToServer cfg :<|> files)
